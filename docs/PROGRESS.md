...
| Per-dictionary column maps | Backlog | Each dictionary's positional columns mapped to named fields, derived from its header comment and verified against a sample of entries. **Next step.** ~87 dictionaries, but core scope needs only: `skilldict`, `socialskilldict`, `weaponvalueslist`, `armorvalueslist`, `equipvalueslist`, `raceStatsAndMoveDetails` (105 races x 62 cols), `classRequirementsAndDetails` (88 classes), `classtitledict`, `goalupdict`. |
