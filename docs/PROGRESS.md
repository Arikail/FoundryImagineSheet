...
| Reconcile Character attribute maximums with his code | Backlog | Found during the creature audit review, not yet fixed. `ImagineCharacterData._prepareAttributes` uses the lower of a Master's Manual title cap (23/25/27/30) and the race limit. His code instead starts maximums at the race limits (sheet-worker.js:8099) and `setArchMortalAttributesMax` (27549) sets every maximum to 27 at title 11, overriding the race limit. So an arch-mortal whose race limit is under 27 gets a lower maximum in the port than in his sheet, and none of the five places his code assigns `strength_max` uses the 23/25/30 tiers (checked every assignment; the other eleven attributes were not checked separately). DoD: maximums match his code (the sheet wins), with derivation tests covering title 10→11 for a race with limits below 27. |
