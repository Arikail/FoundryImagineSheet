...
| Story | Status | Definition of Done |
|---|---|---|
| Creature actor type & sheet | In Progress | Source-material blocker is resolved (Bestiary material obtained, Epic 0). Full audit of the Roll20 creature sheet (HTML lines 57576-92926) and the sheet-worker's creature-handling code (lines 174658-180370, plus shared functions that branch on creature-vs-character) is done and logged in `docs/DECISIONS.md` ("Creature/NPC audit findings"). Confirmed reusable as-is: the 12 attributes, the body-area/wound/armor system (`BODY_CHARTS`/`createBodyAreas` already cover every stock creature body type), and attack resolution (`getAttackChart`/`resolveAttack`/`resolveFumble`). Confirmed creature-specific and needing new schema: identity (type/subtype/level/etc.), a level-tiered attribute cap, characteristics with mixed derivation (Endurance/Hide flat, Shock conditionally derived, Perception/Affinity/Fortune derived on a creature-only formula), flat-plus-modifier resistances, the 10-slot natural-attack block (custom encoded format, not a weapon Item, with non-physical attack shapes), and Ability/Disability/Immunity as flavor-text lookups rather than mechanical data. **Blocked on:** the model-choice check-in (Opus/Fable) required before real schema implementation per the working protocol, and three open design decisions logged in `DECISIONS.md` that are the user's call (Ability/Disability/Immunity as plain Items vs. effect-granting Items; Powers vs. Attacks as one item type or two; whether a tamed-creature/owner relationship field is in scope now). |
