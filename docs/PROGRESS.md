...
| Design Actor data model (Character, Creature) | Backlog | Schema covers all fields cataloged from the Roll20 sheet's core tabs; documented against source page refs in `docs/reference/`. **Real design work — flagged as an Opus task per the model protocol, not yet started.** |
