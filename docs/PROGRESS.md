...
| Story | Status | Definition of Done |
|---|---|---|
| Define Foundry system manifest (`system.json`) targeting V14 | Backlog | Manifest validates, loads in Foundry V14 as an empty system. |
| Design Actor data model (Character, Creature) | Backlog | Schema covers all fields cataloged from the Roll20 sheet's core tabs; documented against source page refs in `docs/reference/`. |
...
| Design content/compendium layer (sourcebook tagging, enable/disable) | Backlog | A compendium entry can declare a sourcebook; a world-level setting can enable/disable a sourcebook or override an individual entry; documented data shape. |
